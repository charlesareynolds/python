...
# 2007/12/21 CAR Adding specific AccuRev migration directory structure tweaks:
#            Eliminate IDL directory in target

__author__ = "Charles Reynolds"
__version__ = "$Revision: 1.34 $"
__date__ = "$Date: 12/20/2007 09:16:05 $"
...
def Link_A_File (Source_Path, Target_Path):
    ...
    so they must use relative (not absolute) symbolic links.
    """
    global Target_Count
    Debug_Print ("Link_A_File (" + Source_Path + ", " + Target_Path + ")")
    Source_Path = os.path.normpath (Source_Path)
    Source_Parts = Source_Path.split ("/")
    Target_Path = os.path.normpath (Target_Path)
    Target_Parts = Target_Path.split ("/")
    Shortest_Path_Length = min (len (Source_Parts), len (Target_Parts))
    # Eliminate common root parts, but keep the file name:
    for index in range (Shortest_Path_Length - 1):
    ...
    # Down from Target_Path, not counting the file name:
    for index in range (len (Target_Parts) - 1):
        Relative_Source_Path = "../" + Relative_Source_Path
    # Up to Source_Path, including file name:
    Relative_Source_Path = Relative_Source_Path + "/" + "/".join (Source_Parts)
        ...
        if not os.path.isdir(Source_Entry_Path):
            if Is_Ada_File (Entry):
                if options.Make_Both_Format_Targets or options.Make_Gnat_Format_Targets:
                    Target_Entry_Path = os.path.join (Target_Path, Apex_To_Gnat(Entry))
                    Copy_Or_Link_A_File (Source_Entry_Path, Target_Entry_Path)
                if options.Make_Both_Format_Targets or (not options.Make_Gnat_Format_Targets):
                    Target_Entry_Path = os.path.join (Target_Path, Gnat_To_Apex(Entry))
                    Copy_Or_Link_A_File (Source_Entry_Path, Target_Entry_Path)
...
def Link_IDL_Files (Source_Path, Target_Path):
    ...
    # Trim the IDL off the target path: 
    if os.path.basename (Target_Path) == IDL_Source_Dir:
    ...
    for Entry in os.listdir (Source_Path):
        ...
        if not os.path.isdir(Source_Entry_Path):
            if Entry [-4:] == IDL_Suffix:
                Target_Entry_Path = os.path.join (Target_Path, Entry)
                Copy_Or_Link_A_File (Source_Entry_Path, Target_Entry_Path)
                Target_Entry_Link_Path = os.path.join (Target_IDL_Links_Path, Entry)
                Link_A_File (Target_Entry_Path, Target_Entry_Link_Path)
                Files_Copied_Or_Linked = Files_Copied_Or_Linked + 1
            Files_Processed = Files_Processed + 1 
...
def Process_View_Dirs (View_Path, Target_Path, The_Process):
    global Apex_Internal_Dirs
    """Recursively processes the directories starting at View_Path, leaving out the internal 
    Rational dirs.  Sends each along with its corresponding Target directory to The_Process.
    """
    Debug_Print ("Process_View_Dirs (" + View_Path + ", " + Target_Path + ")")
    # Actual processing: 
    The_Process (View_Path, Target_Path)
    ...
    for Entry in Entries:
        ...
        if os.path.isdir (Entry_Path):
            ...
            # Recurse:
            #
            Process_View_Dirs (Entry_Path, os.path.join (Target_Path, Entry), The_Process)
...
def Process_Subsystems (View, The_Process, Layer_Path, Target_Layer_Path, Subsystems):
    ...
    for Subsystem in Subsystems:
        ...
        # Trim ".ss" off the end of the target subsystem name:
        Target_Path = os.path.join (Target_Layer_Path, Subsystem [:-3])
        # Debug_Print ("Process_Subsystems: View_Path   => " + View_Path)
        # Debug_Print ("Process_Subsystems: Target_Path => " + Target_Path)
        # If this is an existing view, process it:
        if os.path.isdir (View_Path):
            Process_View_Dirs (View_Path, Target_Path, The_Process)
...
#    Process_NIF_Views (View_Name, Print_Code_Files)
    ...
    # # put the resulting links in the regular shadow tree:
    # Process_Subsystems (
    ...
    Target_IDL_Links_Path = os.path.join (Target_IDL_Root_Path, "links")
    # Two directories are targets for IDL.  Only one is deleted by
    # Process_NIF_Views.  Delete the other one:
