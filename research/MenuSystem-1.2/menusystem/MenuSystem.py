...
class Menu:
	...
	level menu object.  The calls will trickle down to all of the menu objects
	...
	def waitForInput(self):
		...
		while repeat != False:
			...
			if c in self.choices:
				...
				if choice.subMenu:
					# Recurse first:
					choice.subMenu.waitForInput()
					if choice.handler:
						# Call the handler with the result of the submenu:
						repeat = choice.handler(choice.subMenu.data)
				else:
					if choice.handler:
						# Call the handler with the choice's stored value:
						repeat = choice.handler(choice.value)
			else:
				print "*** Choice '" + c + "' not found."
...
class DataMenu(Menu):
	...
	def waitForInput(self):
		...
		specifies it, (with any trailing whitespace including cr or lf stripped)
		"""
		print self,
		local_line = sys.stdin.readline().rstrip()
		if local_line != "":
			self.data = local_line
...
class Choice:
	...
	def __eq__(self, ch):
		...
		return isinstance(ch, str) and (ch == str(self.selector))
...
class MenuGenie:
	...
	def save(self, menu):
		...
		Write an existing Menu object and all sub-mens to permnt storage.
