...
class XMLMenuGenie(menusystem.MenuGenie):
	...
	XML files.  The format of the XML file is as follows:
		...
		<menu>  -  represents a menu object (valid anywhere).  Must contain at
			...
			attribute list
				title  -  menu title
				prompt -  menu prompt
				isDataMenu   -  Is this a data menu?
	...
	def __init__(self, loc, module_name):
		"""Initialize the XMLGenie Object
	...
	def _load(self, head):
		"""Creates all menu objects recursively
		
		Starting at the document root, processes all menu tags thus building
		the menu system.
		
		Each menu tag is processed for it's attributes and contained choices.
		If one of the menu's choices is itself a [sub]menu, then recursively
		process all of that menu's attributes and choices.
		
		The parameter to this function is the xml an menu tag object.  Start 
		with the document root, and recursively progress down through submenus.
		...
		if head.getAttribute('isDataMenu').lower() == 'true':
	...
	def _save(self, menu):
		...
		m.setAttribute('isDataMenu', 'False')
		try:
			...
			m.setAttribute('isDataMenu', 'True')
		return m
